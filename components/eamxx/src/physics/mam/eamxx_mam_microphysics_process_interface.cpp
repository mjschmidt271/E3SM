...
void MAMMicrophysics::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
...

  {
    ...
    for (const auto &var_name : extfrc_lst_) {
      ...
      EKAT_REQUIRE_MSG(nvars <= int(mam_coupling::MAX_SECTION_NUM_FORCING),
      ...
      for (int isp = 0; isp < nvars; ++isp) {
        forcings_[i].fields[isp] = view_2d("elevated_emis_output_", ncol_, nlev_);
      }
      ++i;
    } // end i
  ...
  if (extra_mam4_aero_microphys_diags_) {
    ...
    const std::map<std::string, std::string> mixed_units_fields = {
        {"mam4_microphysics_tendency_gas_phase_chemistry",
         "MAM4xx microphysics tendencies due to gas phase chemistry [mixed units: kg/kg/s or "
         "#/kg/s]"},

        {"mam4_microphysics_tendency_aqueous_chemistry",
         "MAM4xx microphysics tendencies due to aqueous chemistry [mixed units: kg/kg/s or "
         "#/kg/s]"},
        ...
        {"mam4_microphysics_tendency_condensation",
         "MAM4xx microphysics tendencies due to gas aerosol exchange (condensation) [mixed units: "
         "mol/mol/s or #/mol/s]"},

        {"mam4_microphysics_tendency_renaming",
         "MAM4xx microphysics tendencies due to gas aerosol exchange (renaming) [mixed units: "
         "mol/mol/s or #/mol/s]"},

        {"mam4_microphysics_tendency_nucleation",
         "MAM4xx microphysics tendencies due to gas aerosol exchange (nucleation) [mixed units: "
         "mol/mol/s or #/mol/s]"},

        {"mam4_microphysics_tendency_coagulation",
         "MAM4xx microphysics tendencies due to gas aerosol exchange (coagulation) [mixed units: "
         "mol/mol/s or #/mol/s]"},

        {"mam4_microphysics_tendency_renaming_cloud_borne",
         "MAM4xx microphysics tendencies due to gas aerosol exchange (renaming cloud borne) [mixed "
         "units: mol/mol/s or #/mol/s]"},
...
void MAMMicrophysics::run_impl(const double dt) {
  ...
  view_3d gas_aero_exchange_condensation, gas_aero_exchange_renaming, gas_aero_exchange_nucleation,
  ...
  if (extra_mam4_aero_microphys_diags_) {
    ...
    gas_phase_chemistry_dvmrdt =
        get_field_out("mam4_microphysics_tendency_gas_phase_chemistry").get_view<Real ***>();
    aqueous_chemistry_dvmrdt =
        get_field_out("mam4_microphysics_tendency_aqueous_chemistry").get_view<Real ***>();
    aqso4_incloud_mmr_tendency =
        get_field_out("mam4_microphysics_tendency_aqso4").get_view<Real ***>();
    aqh2so4_incloud_mmr_tendency =
        get_field_out("mam4_microphysics_tendency_aqh2so4").get_view<Real ***>();
    gas_aero_exchange_condensation =
        get_field_out("mam4_microphysics_tendency_condensation").get_view<Real ***>();
    gas_aero_exchange_renaming =
        get_field_out("mam4_microphysics_tendency_renaming").get_view<Real ***>();
    gas_aero_exchange_nucleation =
        get_field_out("mam4_microphysics_tendency_nucleation").get_view<Real ***>();
    gas_aero_exchange_coagulation =
        get_field_out("mam4_microphysics_tendency_coagulation").get_view<Real ***>();
    gas_aero_exchange_renaming_cloud_borne =
        get_field_out("mam4_microphysics_tendency_renaming_cloud_borne").get_view<Real ***>();
  }
  ...
  for (const auto &var_name : extfrc_lst_) {
    ...
    scream::mam_coupling::advance_tracer_data(
        ...
        elevated_emiss_time_state_[i], elevated_emis_data_[i], dry_atm_.p_mid, dry_atm_.z_iface,
      ...
      "MAMMicrophysics::run_impl", policy, KOKKOS_LAMBDA(const ThreadTeam &team) {
        ...
        if (extra_mam4_aero_microphys_diags) {
          ...
          diag_arrays.aqh2so4_incloud_mmr_tendency =
              ekat::subview(aqh2so4_incloud_mmr_tendency, icol);

          diag_arrays.gas_aero_exchange_condensation =
              ekat::subview(gas_aero_exchange_condensation, icol);
          ...
          diag_arrays.gas_aero_exchange_nucleation =
              ekat::subview(gas_aero_exchange_nucleation, icol);
          diag_arrays.gas_aero_exchange_coagulation =
              ekat::subview(gas_aero_exchange_coagulation, icol);
          diag_arrays.gas_aero_exchange_renaming_cloud_borne =
              ekat::subview(gas_aero_exchange_renaming_cloud_borne, icol);
        }
        ...
        mam4::microphysics::perform_atmospheric_chemistry_and_microphysics(
            ...
            solar_flux, cnst_offline_icol, forcings_in, atm, photo_table, config.setsox,
            config.amicphys, zenith_angle(icol), d_sfc_alb_dir_vis(icol), o3_col_dens_i,
            photo_rates_icol, extfrc_icol, invariants_icol, work_photo_table_icol, config.linoz,
            linoz_data, eccf, adv_mass_kg_per_moles, fraction_landuse_icol, index_season, clsmap_4,
            permute_4, offset_aerosol, dry_diameter_icol, wet_diameter_icol, wetdens_icol,
            dry_atm.phis(icol), cmfdqr, prain_icol, nevapr_icol, work_set_het_icol, drydep_data,
  ...
  Kokkos::parallel_for(
      "transpose_extfrc", Kokkos::MDRangePolicy<Kokkos::Rank<3>>({0, 0, 0}, {ncol, extcnt, nlev}),
